...
- [x] **3.1 Create `MyFilesFolderListItem.tsx` component**
  - [x] Move only the folder list-view row JSX
  - [x] Keep folder state, selection behavior, drag/drop behavior, navigation, and action menu logic owned by `MyFiles.tsx`
  - [x] Leave folder grid, file list, file grid, action menu, modal, and final cleanup tasks unchecked
  - [ ] Verify folder list row rendering and interactions

- [x] **3.1a Create `MyFilesFolderGridItem.tsx` component**
  - [x] Move only the folder grid-view card JSX
  - [x] Keep folder state, selection behavior, drag/drop behavior, navigation, and action menu logic owned by `MyFiles.tsx`
...
- [x] **3.1c Create `MyFilesFileGridItem.tsx` component**
  - [x] Move only the file grid-view card JSX
  - [x] Keep file selection behavior, drag/drop behavior, preview/share/download/move/delete behavior, and action menu logic owned by `MyFiles.tsx`
  - [x] Leave global action menu, modal, and final cleanup tasks unchecked
...
- [x] **3.1e Create `MyFilesFileSection.tsx` component**
  - [x] Extract only the file section wrapper/composition JSX from `MyFiles.tsx`
  - [x] Keep the file section header/title, select-all visible files checkbox, selected-file bulk actions, loading/error/empty states, and list/grid container composition in the new component
...
- [x] **3.1h Create `MyFilesFileActionMenu.tsx` component**
  - [x] Extract only the floating File Action Menu panel JSX from `MyFiles.tsx`
  - [x] Keep file menu state, positioning, trigger button, callbacks, API behavior, and modal triggers owned by `MyFiles.tsx`
...
- [x] **3.1j Create `MyFilesFolderDeleteModal.tsx` component**
  - [x] Extract only the single-folder delete confirmation modal JSX from `MyFiles.tsx`
  - [x] Note: Only the presentational JSX was moved. `isDeleteModalOpen`, `selectedFolderForDelete`, `deleteLoading`, `deleteError`, `handleConfirmDeleteFolder`, and `setOpenFolderActionId(null)` remain owned by `MyFiles.tsx`.
  - [ ] Keep file delete modal, bulk delete modals, rename modals, preview/share/move modals, and final cleanup tasks unchecked
  - [ ] Keep validation checklist unchecked because the user must run `npm run build` manually after the patch

- [x] **3.1k Create `MyFilesFileRenameModal.tsx` component**
  - [x] Extract only the File Rename modal JSX from `MyFiles.tsx`
  - [x] Note: Only the presentational JSX was moved. `isFileRenameModalOpen`, `fileRenameName`, `fileModalError`, `fileActionLoading`, and `handleSubmitFileRename` remain owned by `MyFiles.tsx`.
...
- [x] **3.1p Create `MyFilesPreviewModal.tsx` component**
  - [x] Extract only the Preview modal overlay/container shell from `MyFiles.tsx`
  - [x] Note: Only the presentational overlay and container shell were moved. Preview state, refs, content branch rendering, zoom/pan handlers, close/minimize/maximize/download handlers, preview URL/blob/text logic, and API behavior remain owned by `MyFiles.tsx`.
...
- [x] **3.1r Create `PreviewFallbackFrame.tsx` component**
  - [x] Extract only the fallback/unsupported preview iframe JSX from `MyFiles.tsx`
  - [x] Note: Only the presentational fallback iframe markup was moved. Preview URL state, file name state, content type branching, preview modal state, and preview behavior remain owned by `MyFiles.tsx`.
  - [ ] Keep image/text/video/audio preview branch extraction and final cleanup tasks unchecked
  - [ ] Keep validation checklist unchecked because the user must run `npm run build` manually after the patch

- [x] **3.1s Create `PreviewAudioPlayerFrame.tsx` component**
  - [x] Extract only the audio preview branch JSX/component usage from `MyFiles.tsx`
  - [x] Note: Only the presentational audio player markup was moved. Preview URL state, content type branching, preview modal state, error behavior, and preview behavior remain owned by `MyFiles.tsx`.
...
- [x] **3.1n Create `MyFilesBulkFolderDeleteModal.tsx` component**
  - [x] Extract only the Bulk Folder Delete modal JSX from `MyFiles.tsx`
  - [x] Note: Only the modal overlay, delete confirmation card, selected folder count text, loading state UI, result summary UI, and buttons were moved.
  - [x] Note: `isBulkFolderDeleteModalOpen`, `bulkFolderDeleteIds`, `bulkFolderDeleteLoading`, `bulkFolderDeleteResult`, `handleConfirmBulkFolderDelete`, `closeBulkFolderDeleteModal`, result calculation, and API behavior remain owned by `MyFiles.tsx`.
  - [ ] Keep preview/share/move modals and final cleanup tasks unchecked
  - [ ] Keep validation checklist unchecked because the user must run `npm run build` manually after the patch
